...
export type Triggers<DataModel extends GenericDataModel> = {
  [TableName in TableNamesInDataModel<DataModel>]?: {
    ...
    triggers: FunctionReference<"mutation", any, TriggerArgs<DataModel, TableName>, null>[],
  }
};

type TriggerHandles<DataModel extends GenericDataModel> = {
  [TableName in TableNamesInDataModel<DataModel>]?: {
    atomicMutators: { [Mutator in keyof AtomicMutators]: FunctionHandle<"mutation"> };
    triggers: FunctionHandle<"mutation", TriggerArgs<DataModel, TableName>, null>[];
  }
};

export type WithTriggers<DataModel extends GenericDataModel> = {
  args: Record<string, never>;
  input: (ctx: GenericMutationCtx<DataModel>, args: any) => Promise<{ args: Record<string, never>; ctx: { db: WrapWriter<DataModel> } }>;
};

export function withTriggers<
  DataModel extends GenericDataModel,
>(api: InstalledAPI, triggers: Triggers<DataModel>): WithTriggers<DataModel> {
  return {
    args: {},
    input: async (ctx: GenericMutationCtx<DataModel>, _args: any) => {
      ...
      for (const tableNameStr of Object.keys(triggers)) {
        ...
        handles[tableName] = {
          atomicMutators: {
            atomicInsert: await createFunctionHandle(tableTrigger.atomicMutators.atomicInsert),
            atomicPatch: await createFunctionHandle(tableTrigger.atomicMutators.atomicPatch),
            atomicReplace: await createFunctionHandle(tableTrigger.atomicMutators.atomicReplace),
            atomicDelete: await createFunctionHandle(tableTrigger.atomicMutators.atomicDelete),
          },
          triggers: await Promise.all(tableTrigger.triggers.map(createFunctionHandle)),
        };
      }
...
> {
  ...
  async delete<TableName extends string>(table: TableName, id: GenericId<TableName>): Promise<void> {
    if (table in this.triggers) {
      const tableTrigger = this.triggers[table]!;
